...
-집합자료형
    set을 사용하며 크게 두가지의 특징이 있다.
    1. 중복을 허용하지 않는다
    2. 순서가 없다 > 인덱싱 불가
    사용 함수로는 교집합(&), 합집합(|), 차집합(-)등이 있다.
    중복을 허용하지 않는 set의 특성을 이용해 어떤 리스트를 중복없이 나열할때도 set을 사용하곤 한다.


-bool 자료형
    bool자료형이란 True와 False를 나타내는 자료형으로서 비교연산자(조건문)를 통해 나오기도 하고 여러가지 방법이 있다.
    예시로 자료형에도 bool값이 존재하는데 이를테면 ''와 같은 빈 문자열은 False, 숫자나 '3'같은 문자열은 True이다.
    비교연산자 >,<,== 등을 이용해도 bool값을 출력할 수 있다. bool() 함수는 괄호안의 자료형이 참인지 거짓인지 판단해준다.

-변수
    변수는 변하는 값을 저장하는 하나의 공간이라고 생각하면 된다. 변수 선언 조건은 특수문자나 숫자로 시작하면 안된다.
    특이한 예로 리스트를 변수로 저장했는데 새로운 변수에 그 변수를 대입하면 리스트가 저장된 두개의 변수의 리스트 모두 
    요소값이 변경된다.
-변수를 선언하는 여러가지 방법
    a,b= 3,5
    a=b=5
    a,b = b,a 를 할시 두개의 값이 바뀜.